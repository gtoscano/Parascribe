...
(server-side running$\rightarrow$finished); GPU memory and utilisation sampled
at 2\,Hz via \texttt{nvidia-smi}; and host RAM. We report throughput in
...
times. Each invocation records a fresh 2-second idle baseline, and multi-level
...
\msd{mean}{SD}; speedups are ratios of repeated means. All retained trials had
zero failed jobs. The raw per-trial CSV files accompany the paper in
\texttt{measurements/repetitions/}.
...
\begin{lstlisting}[language=]
# Set `- WORKERS=2` in docker-compose-whisperx.yml, then:
