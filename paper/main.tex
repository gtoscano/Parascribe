...
time of a batch of $N$ files on $R$ replicas. The model predicts that
throughput is \emph{non-monotonic} in $N$, peaking whenever $R$ divides $N$
and sagging on a partial wave; a sweep at fixed $R$ confirms this, with
throughput at $N{=}4$ recovering to a value statistically indistinguishable
from $N{=}2$ and predicted wall times within $1.4\%$ of measurement without a
fitted parameter. Three end-to-end trials at $N{=}10$ show that the
synchronized-wave estimate is conservative for continuously queued work,
...
\begin{itemize}
  ...
  estimates homogeneous batch completion time from concurrency-specific
  service times, and predicts the non-monotonic throughput that follows when
  the in-flight job count is not a multiple of the replica count.
\end{itemize}
