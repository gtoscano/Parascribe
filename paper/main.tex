...
and shortening later service times. Equation~\eqref{eq:model2} is therefore a
...
% ===========================================================================
% ===========================================================================
% DRAFT — proposed addition to \section{Results}.
% Insert after \subsection{No gateway penalty is observed}.
% Data: measurements/ami_2rep_1gpu_l4r5/, n=5 per level.
% ===========================================================================

\subsection{Over-subscription is governed by wave quantisation}
\label{sec:oversub}

The preceding sections vary the number of replicas with offered concurrency
held equal to it. A deployment does not get that luxury: the
replica count is fixed at provisioning time and the arrival rate is not. We
therefore fix a two-replica deployment and vary only the offered concurrency
$c$ (Table~\ref{tab:oversub}).

Throughput is not monotonic in $c$. It rises from $c=1$ to $c=2$, \emph{falls}
at $c=3$, and returns at $c=4$ to a value statistically indistinguishable from
$c=2$ (the 95\% intervals $[8.372, 8.392]$ and $[8.304, 8.416]$ overlap). A
dip that reverses is not contention, which would worsen monotonically.

The explanation is quantisation. With $R$ replicas each running one job at a
time, $c$ offered jobs execute in $\lceil c/R \rceil$ waves, and a wave costs
what its \emph{widest} occupancy costs. Writing $T_k$ for the wall time of a
wave of $k$ concurrent jobs, the batch completes in
\begin{equation}
T(c) \;=\; \Big\lfloor \tfrac{c}{R} \Big\rfloor \, T_R \;+\;
           \big[\, c \bmod R > 0 \,\big] \; T_{c \bmod R},
\label{eq:waves}
\end{equation}
so throughput $c/T(c)$ peaks whenever $R \mid c$ and sags otherwise, because a
partial final wave occupies the same elapsed time as a full one while
completing fewer jobs. With the measured $T_1 = 10.18$\,s and $T_2 =
14.31$\,s, Equation~\ref{eq:waves} predicts $T(3) = 24.49$\,s and $T(4) =
28.63$\,s against observed $24.15$\,s and $28.71$\,s --- errors of $-1.4\%$ and
$+0.3\%$ with no fitted parameter.

The practical consequence is that a two-replica deployment serving an odd
in-flight count wastes roughly half a wave. Provisioning should therefore match
the replica count to the expected concurrency, or admit jobs in multiples of
$R$; adding replicas that leave $R \nmid c$ can measurably reduce throughput.

Two caveats. First, $c=3$ is also the only unstable level: its coefficient of
variation is $4.2\%$ against $\le 0.5\%$ elsewhere, and its five observations
fall into two clusters ($22.4$\,s once, $24.5$--$24.7$\,s four times) rather
than scattering about the mean. Equation~\ref{eq:waves} assumes the partial
wave starts only once a full wave has drained, whereas the trailing job can in
fact start as soon as the \emph{first} replica frees; the faster cluster is
consistent with greater overlap between waves. We report the effect without
claiming to have characterised it. Second, the mean of a bimodal level
describes a state the system does not occupy, so Table~\ref{tab:oversub}
reports dispersion alongside it.

\begin{table}[t]
\centering
\caption{Offered concurrency against a \emph{fixed} two-replica deployment
(W-short). Throughput is mean (sample SD) with a two-sided 95\% $t$ interval,
$n=5$. Efficiency is speedup over $c=1$ divided by $c$; $e$ is the Karp--Flatt
experimentally determined serial fraction. Throughput at $c=4$ recovers to the
$c=2$ value because both are integer multiples of the replica count.}
\label{tab:oversub}
\begin{tabular}{ccccc}
\toprule
Offered & Throughput & 95\% CI & Efficiency & Karp--Flatt \\
$c$ & (\filesmin) & (\filesmin) & (\%) & $e$ \\
\midrule
1 & \msd{5.896}{0.005} & $[5.889,\,5.903]$ & $99.9$ & --- \\
2 & \msd{8.382}{0.008} & $[8.372,\,8.392]$ & $71.1$ & $0.406$ \\
3 & \msd{7.462}{0.313} & $[7.073,\,7.851]$ & $45.3$ & $0.603$ \\
4 & \msd{8.360}{0.045} & $[8.304,\,8.416]$ & $35.6$ & $0.604$ \\
\bottomrule
\end{tabular}
\end{table}

The Karp--Flatt fraction rises from $0.406$ at $c=2$ to $\approx 0.60$ at
$c=3$ and $c=4$, then holds. Read with Equation~\ref{eq:waves}, the step is
queueing entering the measurement rather than a larger serial section: the
metric attributes the whole of a partial wave's idle time to serialisation,
which is arithmetically correct but conflates two mechanisms. We report it
because a rising $e$ distinguishes overhead from an inherently serial section,
which speedup alone cannot; we do not read the absolute value as a property of
the pipeline.
...
Throughput saturates earlier still, at $c=2$ (\S\ref{sec:oversub}), so neither
